    ...
    <article 
      ...
      <div class="flex justify-between items-start mb-4">
        ...
        @if (project().isHighlight) {
          <span class="text-xs font-semibold px-2 py-1 bg-primary/20 text-primary border border-primary rounded-full">Destaque</span>
        }
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 mt-auto">
        <app-button variant="outline" class="flex-1" (onClick)="openLink(project().repositoryUrl)">Repositório</app-button>
        @if (project().liveUrl) {
          <app-button variant="primary" class="flex-1" (onClick)="openLink(project().liveUrl!)">Ver ao vivo</app-button>
        }
      </div>
    </article>
  `
